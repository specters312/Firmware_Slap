...
class SystemLibc(angr.procedures.libc.system.system):
    def check_exploitable(self, cmd):
        ...
        symbolic_list = [
            self.state.memory.load(
                self.state.se.eval(clarip, cast_to=int) +
                x).get_byte(0).symbolic for x in range(value_length)
        ]
        ...
        if greatest_count >= len("`reboot`"):
            val_loc = self.state.memory.load(location + position,
                                             len("`reboot`"))
            if self.state.satisfiable(
                    extra_constraints=[val_loc == "`reboot`"]):
    ...
    def check_for_constraint(self):
        ...
        for x, y, z in self.state.globals['args']:
            ...
            value_address = str(action.actual_addrs).replace("[", "").replace(
                "]", "").replace("L", "")
            value_address = int(value_address)
            if "`ls`" in value_str or "`reboot`" in value_str:
                return (True, value_str, value_address)
